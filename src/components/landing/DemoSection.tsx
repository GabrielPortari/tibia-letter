  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={`font-mono font-bold text-sm flex-shrink-0 ${secs <= 60 ? 'text-red' : 'text-gold'}`}>
          {fmt(secs)}
        </span>
      </div>
      ...
      {activeHunts.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-1">
          {activeHunts.map((e) => (
            ...
            >
              ⚔ <span className="font-medium">Seraphion</span> {t('banner.hunting_at')}{' '}
        ...
        <div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {waiting.map((e) => (
              ...
              >
                <span className="text-text font-medium">{e.spawnName}</span> — {t('banner.position', { pos: e.position })}
              </span>
            ))}
          </div>
        </div>
...
function SpawnCardMock({ spawn, defaultOpen }: { spawn: MockSpawn; defaultOpen?: boolean }) {
  ...
  const statusLabel = { free: t('spawn.status_free'), occupied: t('spawn.status_occupied'), pending: t('spawn.status_pending') }[spawnStatus]
        ...
        <div className="px-4 pb-4 border-t border-border pt-3 space-y-3 animate-fadeIn">
          ...
          <div className="flex gap-2 flex-wrap">
            {!myEntry && (
              ...
              >
                {spawn.queue.length === 0 ? t('spawn.hunt_now') : t('spawn.join_queue')}
              </button>
            )}
            {isHunting && (
